...
@section('content')
    ...
    <div class="container">
        <div class="user-card">
            ...
            <form method="GET" action="{{ route('users.index') }}"
                style="margin-bottom: 12px !important; display: flex; width: 100%;>
                <div style="display: flex
                !important; flex-direction: row !important; gap: 4px !important; padding: 4px !important; align-items:
                center !important;">
                <!-- Calendar Dropdown -->
                ...
                <select name="type" class="form-wd mb-type" id="type" onchange="this.form.submit()">
                    <option value="all" {{ $type == 'all' ? 'selected' : '' }} data-text="All Users" data-icon="📇">All
                        Users</option>
                ...
                <!-- Name Input -->
                <input type="text" class="name-wd mb-name-search" name="name" placeholder="Name"
                    value="{{ request('name') }}">

                <!-- Email Search with Button -->
                <input type="text" class="name-wd mb-email-search-input" name="email" placeholder="Email"
                    value="{{ request('email') }}">
                <button type="submit" class="btn btn-primary beside-email-search-btn">
    ...
    <script>
        function updateDropdownDisplay() {
            ...
            let isMobile = window.innerWidth <= 768; // Detect mobile screens

            for (let option of select.options) {
                if (isMobile) {
                    option.textContent = option.getAttribute('data-icon'); // Show icon
                } else {
                    option.textContent = option.getAttribute('data-text'); // Show text
                }
            }
        }
