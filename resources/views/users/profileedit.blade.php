...
<div class="container">
    <div class="card shadow border-0">
        ...
        <div class="card-body"> 
            <form id="editProfileForm" method="POST" enctype="multipart/form-data" action="{{ route('profile.update', $user->id) }}">
                @csrf
                @method('PUT')
                ...
                <!-- Email -->
                <div class="row mb-3">
                    <label for="email" class="col-md-4 col-form-label text-md-end">E-mail Address<span class="text-danger">*</span></label>
                    <div class="col-md-6">
                        <input class="form-control @error('email') is-invalid @enderror" id="email" name="email" value="{{ old('email', $user->email) }}">
                        @error('email')
                        <strong class="text-danger">{{ $message }}</strong>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" {{ Auth::user()->type == 1 ? 'disabled' : '' }}>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        <input class="form-control @error('profile') is-invalid @enderror" type="file" id="profile" name="profile" accept="image/*">
                        @error('profile')
                        <strong class="text-danger">{{ $message }}</strong>
                        @enderror
                    </div>
...
<script>
    $(document).ready(function () {
        $("#clearFormBtn").click(function () {
            ...
            $("#profilePreview").attr("src", "{{ $user->profile ? asset('storage/' . $user->profile) : 'https://via.placeholder.com/150' }}");
        });
